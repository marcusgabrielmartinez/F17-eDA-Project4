...
### Boosting
```{r}
boost.parkinsons=gbm(total_updrs~.,data=df_tot_nums[train,],distribution="gaussian",n.trees=2000,shrinkage=0.01,interaction.depth=4)
...
```{r}
n.trees2=seq(from=100,to=2000,by=100)
...
test.err1=double(19)
for(mtry1 in 1:19){
  fit=randomForest(total_updrs~.,data=df_tot_nums,subset=train,ntree=100)
  oob.err1[mtry1]=fit$mse[100]
...
```
Although this error seems to be jumping around quite a bit, it is important to note taht the y axis is only spaning 0.02. The dips in the error may be due to including correlated variables, of which there are a significant number. Boosting does a better job in this regard than random forests.
...
boost.df_sex=gbm(sex2~.,data=df_sex2[train,],distribution="gaussian",n.trees=2000,shrinkage=0.01,interaction.depth=4)
...
```{r}
n.trees3=seq(from=100,to=2000,by=100)
...
test.err2=double(20)
for(mtry2 in 1:20){
  fit=randomForest(sex2~.-sex,data=df_sex,subset=train,mtry=mtry2,ntree=100)
  oob.err2[mtry2]=fit$mse[100]
...
```{r}
boost.age=gbm(age2~.-age,data=df_age[train,],distribution="gaussian",n.trees=2000,shrinkage=0.01,interaction.depth=4)
...
```{r}
n.trees5=seq(from=100,to=2000,by=100)
...
test.err3=double(20)
for(mtry3 in 1:20){
  fit=randomForest(age2~.-age,data=df_age,subset=train,mtry=mtry3,ntree=100)
  oob.err3[mtry3]=fit$mse[100]
